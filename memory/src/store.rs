...
impl Store {
    ...
    ///
    /// **Precondition**: `obs.session_id` must have been registered via
    /// [`Self::register_session`] before calling this function; the
    /// `observations.session_id` column carries a foreign-key constraint.
    ///
    ...
    ///
    /// Returns the row id.
    pub fn save_observation(&mut self, obs: NewObservation) -> rusqlite::Result<i64> {
        let now = iso_now();
        ...
        if let Some(ref topic_key) = obs.topic_key {
            ...
            if let Some(id) = existing_id {
                tx.execute(
                    "UPDATE observations
                     SET session_id = ?1, title = ?2, content = ?3, type = ?4, source = ?5,
                         revision_count = revision_count + 1, updated_at = ?6
                     WHERE id = ?7",
                    params![
                        obs.session_id,
                        obs.title,
...
mod tests {
    ...
    // 5b. soft delete removes row from FTS5 index ---------------------------------

    #[test]
    fn test_soft_delete_removes_from_fts5() {
        let mut store = test_store();
        seed_session(&store, "s1", "proj");
        ...
        let id = store
            .save_observation(make_obs(
                ...
                "proj",
                "Canary title",
                "Canary content xyzzy",
            ))
            .unwrap();

        // Searchable before delete.
        assert_eq!(store.search("xyzzy", None, 10).unwrap().len(), 1);

        store.soft_delete(id).unwrap();

        // The FTS5 index must not contain the soft-deleted row.
        ...
        assert_eq!(
            fts_count, 0,
            "soft-deleted observation should be removed from FTS5 index"
        );
    }

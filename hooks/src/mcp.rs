...
///
/// All current tools are read-only observability queries (analyze, search, get,
/// check, list, query). Read prefixes mirror FR-MR-3's `route_datadog`.
...
mod tests {
    ...
    fn test_datadog_mcp_read_allow() {
        let tools = [
            ...
            "mcp__datadog-mcp__list_datadog_monitors",
            "mcp__datadog-mcp__query_datadog_metrics",
        ];
        for tool in &tools {
            let d = route(tool);
            assert_eq!(d, McpDecision::Allow, "expected ALLOW for {}", tool);

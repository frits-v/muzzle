...
deploy: release
	@if [ -n "$$(git status --porcelain -- hooks/ Cargo.toml Cargo.lock Makefile)" ]; then \
		echo "ERROR: Uncommitted changes in tracked build files."; \
		echo "Commit or stash before deploying."; \
		git status --short -- hooks/ Cargo.toml Cargo.lock Makefile; \
		exit 1; \
	fi
	@echo "Deploying to $(DEPLOY_TARGET)/"
	@mkdir -p $(DEPLOY_TARGET)/bin $(DEPLOY_TARGET)/hooks/src
	@# Binaries
	@for b in session-start permissions changelog session-end ensure-worktree; do \

...
/// config (e.g. `<ws>/ml-upsell/.agents/`, `<ws>/Hermosa/CLAUDE.md`).
...
fn is_config_path(path: &str, ws: &str) -> bool {
    ...
    if let Some(rest) = path.strip_prefix(&ws_prefix) {
        // rest = "ml-upsell/.agents/foo.md" or "Hermosa/CLAUDE.md"
...
mod tests {
    ...
    fn test_worktree_path_allow() {
        ...
        let paths = [
            format!("{}/Hermosa/.worktrees/abc12345/some/file.py", ws_str),
    ...
    fn test_main_checkout_deny() {
        ...
        let ws_str = ws.to_string_lossy();
        let paths = [
            format!("{}/Hermosa/app/main.py", ws_str),
    ...
    fn test_no_worktree_session_deny() {
        ...
        let ws = config::workspace();
        let p = format!("{}/Hermosa/app/main.py", ws.display());
        let result = check_path(&p, Some(&sess));
        assert!(
            matches!(result, PathDecision::Deny(_)),
    ...
    fn test_no_worktree_session_allows_existing_worktree_paths() {
        ...
        let paths = [
            ...
            format!("{}/Hermosa/.worktrees/other-session/app/main.py", ws_str),
        ];
        for p in &paths {
            let result = check_path(p, Some(&sess));
            assert!(
    ...
    fn test_repo_agents_allowed_in_worktree_mode() {
        ...
        let paths = [
            ...
            format!("{}/Hermosa/.agents/handoff/test.md", ws_str),
    ...
    fn test_repo_claude_md_allowed_in_worktree_mode() {
        ...
        let paths = [
            ...
            format!("{}/Hermosa/AGENTS.md", ws_str),
    ...
    fn test_worktree_config_paths_redirected() {
        ...
        let paths = [
            ...
            format!(
                "{}/Hermosa/.worktrees/xyz99999/.agents/handoff/notes.md",
                ws_str
            ),
            // CLAUDE.md / AGENTS.md
    ...
    fn test_worktree_non_agents_still_allowed() {
        ...
        let paths = [
            ...
            format!("{}/Hermosa/.worktrees/abc12345/app/main.py", ws_str),
        ];
        for p in &paths {
            let result = check_path(p, Some(&sess));
            assert!(
